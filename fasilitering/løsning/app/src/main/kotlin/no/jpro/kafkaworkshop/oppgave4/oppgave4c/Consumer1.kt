        ...
        processRecord = { record ->
            ...
            message?.let {
                ...
                if (shouldProcessMessage(incomingMessage)) {
                    val id = incomingMessage["producer1ItemId"]?.asText()
                    val consumerId1Key = consumerId1Mapping[id]
                    val value = incomingMessage["producer1ItemValue"]
                    ...
                    val newMessage = message.copyWithAdditionalData(
                        Rapid.RapidMessage.ParticipatingSystem("consumer1"), mapOf(
                            "consumer1Id" to messageNodeFactory.textNode(consumerId1Key)
                        )
                    )
...
fun shouldProcessMessage(incomingMessage: MessageData): Boolean {
    val producer1Id = incomingMessage["producer1ItemId"]
